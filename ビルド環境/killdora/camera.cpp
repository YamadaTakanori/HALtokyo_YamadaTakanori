...
void UpdateCamera(void)
{
	...
	if (GetKeyboardPress(DIK_Z) || (IsButtonPressed(0, BUTTON_Z)))	// （キーボード：Zが押されている間）または（ゲームパッド：LBボタンが押されている間）
	{// 視点旋回「左」
	...
	if (GetKeyboardPress(DIK_C) || (IsButtonPressed(0, BUTTON_Y)))	// （キーボード：Cが押されている間）または（ゲームパッド：RBボタンが押されている間）
	{// 視点旋回「右」
		g_Camera.rot.y -= VALUE_ROTATE_CAMERA;
		if (g_Camera.rot.y < -XM_PI)
		{
