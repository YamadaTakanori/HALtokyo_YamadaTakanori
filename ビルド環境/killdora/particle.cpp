...
HRESULT InitParticle(void)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		...
		g_aParticle[nCntParticle].spd = 5.0f;
		...
		g_aParticle[nCntParticle].size = PARTICLE_WH;
	}
...
void UpdateParticle(void)
{
...

	{
		for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
		{
			...
			{
					// 弾の移動処理
				g_aParticle[nCntParticle].pos.x -= sinf(g_aParticle[nCntParticle].rot.y) * g_aParticle[nCntParticle].spd;
				g_aParticle[nCntParticle].pos.z -= cosf(g_aParticle[nCntParticle].rot.y) * g_aParticle[nCntParticle].spd;
					...
					// フィールドの外に出たら弾を消す処理
					if (g_aParticle[nCntParticle].pos.x < MAP_LEFT
						|| g_aParticle[nCntParticle].pos.x > MAP_RIGHT
						|| g_aParticle[nCntParticle].pos.z < MAP_DOWN
						|| g_aParticle[nCntParticle].pos.z > MAP_TOP)
					{
						g_aParticle[nCntParticle].bUse = FALSE;
...
int SetParticle(XMFLOAT3 pos, XMFLOAT3 rot, XMFLOAT4 col, float fSizeX, float fSizeY, int nLife)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].bUse)
		{
			...
			g_aParticle[nCntParticle].nLife = nLife;
			g_aParticle[nCntParticle].bUse = TRUE;
			g_aParticle[nCntParticle].spd = 10.0f;
