...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* Ａボタン
		if ( dijs.rgbButtons[0] & 0x80 )	padState[i] |= BUTTON_A;		// ゲームパッド：A
		//* Ｂボタン
		if ( dijs.rgbButtons[1] & 0x80 )	padState[i] |= BUTTON_B;		// ゲームパッド：B
		//* Ｃボタン
		if ( dijs.rgbButtons[2] & 0x80 )	padState[i] |= BUTTON_C;		// ゲームパッド：X
		//* Ｘボタン
		if ( dijs.rgbButtons[3] & 0x80 )	padState[i] |= BUTTON_X;		// ゲームパッド：Y
		//* Ｙボタン
		if ( dijs.rgbButtons[4] & 0x80 )	padState[i] |= BUTTON_Y;		// ゲームパッド：LB
		//* Ｚボタン
		if ( dijs.rgbButtons[5] & 0x80 )	padState[i] |= BUTTON_Z;		// ゲームパッド：RB
		//* Ｌボタン
		if ( dijs.rgbButtons[6] & 0x80 )	padState[i] |= BUTTON_L;		// ゲームパッド：BACK
		//* Ｒボタン
		if ( dijs.rgbButtons[7] & 0x80 )	padState[i] |= BUTTON_R;		// ゲームパッド：START
		//* ＳＴＡＲＴボタン
		if ( dijs.rgbButtons[8] & 0x80 )	padState[i] |= BUTTON_START;	// ゲームパッド：スティックL押し込み
		//* Ｍボタン
		if ( dijs.rgbButtons[9] & 0x80 )	padState[i] |= BUTTON_M;		// ゲームパッド：スティックR押し込み
